...
- 脚本：[5g_runtime.sh](X5G/5g_runtime.sh)
- 服务：[5g-runtime.service](X5G/5g-runtime.service)
...
- 5G 模组已正确识别，且对应设备名与连接名已确认

## 参数配置说明

以下参数定义在 [`5g_runtime.sh`](X5G/5g_runtime.sh) 顶部，可按现场环境调整。
...
| `PRECHECK_TIMEOUT_SEC` | `600` | 单项预检查超时时间，单位秒 |
| `REBOOT_ON_FAIL` | `0` | 失败处理开关，`1` 表示失败后重启系统，`0` 表示只记录告警不重启 |
...
```text
2026-04-01 10:00:00 [CHECK] 1    | OK         | OK       | OK         | -        | 10.0.0.2
```
...
3. 修改 [`5g_runtime.sh`](X5G/5g_runtime.sh)
